...
dat_e3 <- dplyr::tibble(N = N, M = as.character(M), I = as.character(I), L)

ggplot(data = dat_e3,
       mapping = aes(x = N, y = log(L), colour = I)) +
  geom_point() +
  geom_smooth(method ="lm") +
  facet_wrap(~ M) +
  theme_minimal()
...
```{r}
# fit the model
lm_e3 <- lm(log(L) ~ N + M + I + N:M + I:M + I:N + M:I:N, data = dat_e3)
summary(lm_e3)
```
...
simulate_e3 <- function(
  ...
  # simulate the expected log plant biomass
  mu <- (exp((beta1 * N) + (beta2 * M) + (beta3 * I) + (beta4 * N * M) + (beta5 * I * M) + (beta6 * I * N) + (beta7 * N * M * I)))

  # simulate the observed log plant biomass
  L <- alpha*(mu + rnorm(n = length(N), 0, sigma_residual))
