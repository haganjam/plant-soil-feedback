...
saveRDS(object = sum_table,
        file = file.path(figure_table_path, 
                         paste0("summary-tbl-exp1-", figure_suff(i_species_sel, exclude_high_N), ".rds")))
```
...
saveRDS(object = lrt_tbl,
        file = file.path(figure_table_path, 
                         paste0("lrt-tbl-exp1-", figure_suff(i_species_sel, exclude_high_N), ".rds")))
```

We also export the full type III anova table.
...
saveRDS(object = aov_tbl,
        file = file.path(figure_table_path, 
                         paste0("aov-tbl-exp1-", figure_suff(i_species_sel, exclude_high_N), ".rds")))
```
...
saveRDS(object = best_mod_tbl,
        file = file.path(figure_table_path, 
                         paste0("best-mod-tbl-exp1-", figure_suff(i_species_sel, exclude_high_N), ".rds")))
```
...
Important note is that the model is fit with data that is translated to the minimum (by substracting log(4)) from the N data. This makes the intercept more easy to interpret.
...
Based on the nitrogen gradient (4, 8, 16, 32 and 64 mg N), nitrogen values under 20 are considered N-limited and nitrogen values over 30 are considered P-limited. Given this difference, we also estimate the contrast between microbes and no microbes under N-limitation (4 - 20 mg N) and under P-limitation (30 - 64 mg N) for natives and invasives.

However, the model was fit on data where the nitrogen data was translated to zero by subtracting log(4). Therefore, we need to take this into account.

So, for N-limitation, we marginalise over: ln(4) to ln(20) which, for the model is: ln(4) - ln(4) to ln(20) - ln(4). Similarly, for the P-limitation, we marginalise over: ln(30) - ln(4) to ln(64) - ln(4).
