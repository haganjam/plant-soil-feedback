...
cv_mod <- compare_models_loocv(data = dat_ex1_mod, formulas = model_list) 
...
lrt_analysis <- function(model) {
  ...
  # extract full model
  full_model <- lm(model_list[[model]], data = dat_ex1_mod)
...
```{r}
# plot the contrast
p_n <- mm_exp1_plot(x_dat = estm_marginal_object$estm, 
                    y_var = "P",
...
mm_exp1_plot_sensitivity(x_dat = estm_marginal_df, 
                         y_var = "P", 
                         colour_var = "model",
