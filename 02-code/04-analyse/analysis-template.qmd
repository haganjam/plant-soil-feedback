...
pred_ex1 <-
  predict(lm_ex1, newdata = pred_ex1_mod, interval = "prediction") |> 
  as.data.frame() |>
  dplyr::tibble()
...
exp1_plot <- function(x_dat, x_pred, P_sel, y_lims) {
...
p_n <- exp1_plot(x_dat = dat_ex1_plot, x_pred = pred_ex1_plot, 
                 P_sel = "Native", y_lims = c(0.4, 2.35))
  
# invasive plot
p_i <- exp1_plot(x_dat = dat_ex1_plot, x_pred = pred_ex1_plot, 
                 P_sel = "Invasive", y_lims = c(0.4, 2.35))
...
## function for plotting
sim_exp_plot <- function(x_dat, M_sel, y_lims, col1, col2) {
...
```{r}
# specify the data-file name
dat_name <- "ex2-test-data.rds"
```

```{r}
# load the data
dat_ex2 <- readRDS(here::here(file.path("data", dat_name)))
...
## function for plotting
exp2_plot <- function(x_dat, x_pred, M_sel, y_lims, col1, col2) {
